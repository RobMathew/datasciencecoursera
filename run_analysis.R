...
#Check data frame dimension sizes
dim(data.x1)
dim(data.y1)
dim(subject1)
#Note they both have the same number of rows, but different columns

# add subject id and the activity factor to the test dataset
...
dim(data.y2)
dim(subject2)
#same number of rows, but different columns here as well
...
# Reshape the data into a tidy narrow dataset, that will have four columns: the subject id, the activity, the measurement (ex. tBodyAccmeanX) and the value of the measurement

# this yields a datatable with 679,734 rows, and four columns
